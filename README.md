...
```
Note: case01-test expects a VM SSH key at `${VM_RUNNER_KEYS_DIR_HOST}/id_ed25519` on the host.
Note: `make ansible-swarm` auto-labels nodes with `vm-capable=true` if `/dev/kvm` exists.
